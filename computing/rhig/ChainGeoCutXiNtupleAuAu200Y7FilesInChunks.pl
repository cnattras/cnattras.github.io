...
#1.  Macro to load libraries (roughly line 11)
#2.  Input/output directories (roughly lines 15 & 17)
#3.  Macro name and location (roughly lines 101 & 218)
#4.  The name of the TTree you are chaining (The sample in this file has the name "Xi".  This file only chains together one TTree properly.)
#to test without running comment out the lines beginning with "system" at approximately line 219
#To change the nodes which this submits to change the array and the number of nodes at approximatley lines 210 and 211
...
$outfiledir = "/data3/nattrass/CuCu200/geoCuts/partialChained";
#the command below should return the fiels which need to be chained together
@dir_array  = `ls /data3/nattrass/CuCu200/geoCuts/xi-cut-*.root`;

$max_lines = $#dir_array; 
#this is the starting file number minus 1. By default the output starts with file number 1.
$firstFile = 0;
$id=$firstFile-1;
