#ifndef StV0GeometricCuts_h
#define StV0GeometricCuts_h
...
class StV0GeometricCuts : public StMaker {
 public:
  StV0GeometricCuts(char *mydataset);
  ~StV0GeometricCuts(){;}
  ...
  void DoLowPtV0s(){doLowPt = true;}
  //void FillPhiHisto(int partID, float pt, float phi, int centBin);
  void FillCutHistos(int partID, float pt,float mass, float DcaV0ToPV, float DcaNeg, float DcaPos, float DcaDaughters, float NHitsNeg, float NHitsPos, float NSigNeg, float NSigPos, float DecayLength); 
 ...
 private:
  ...
  static float histoBoundsX[3][2];//partID, xlow/xhigh
  static float histoBoundsY[9][3][2];//partID,cut, ylow/yhigh
  static int nBins;
  ClassDef(StV0GeometricCuts,1)
};
#endif
