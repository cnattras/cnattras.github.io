#ifndef RawSpectraMaker_h
#define RawSpectraMaker_h
...
class RawSpectraMaker : public StMaker {
 public:
  RawSpectraMaker(char *mydataset);
  ~RawSpectraMaker(){;}
  ...
  void FillPhiHisto(int partID, float pt, int centBin);
 ...
 private:
  ...
  char *GetHistoName(int partID, int centBin);
  char *GetHistoTitle(int partID, int centBin);

  ClassDef(RawSpectraMaker,1)
};
#endif
